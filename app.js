const QUESTIONS = [
 ...
 {id:"e1",topic:"Exponentials & Logs",difficulty:"Warm-up",type:"text",prompt:"Solve 3 · 2ˣ = 48.",answer:["4","x=4","x = 4"],lesson:"Isolate the exponential expression, then match powers or use logs. Example: 5·3ˣ=135 gives 3ˣ=27=3³, so x=3.",explanation:"Divide by 3: 2ˣ=16=2⁴, so x=4.",mistake:"Exponent rule error"},
 {id:"e2",topic:"Exponentials & Logs",difficulty:"Medium",type:"mc",prompt:"Rewrite log₂(32)=5 in exponential form.",choices:["5²=32","2⁵=32","32²=5","2³²=5"],answer:"2⁵=32",lesson:"logₐ(b)=c means aᶜ=b. The base stays the base. Example: log₃(81)=4 becomes 3⁴=81.",explanation:"The base 2 raised to the exponent 5 equals 32.",mistake:"Log form confusion"},
...
function showView(name){$$(".view").forEach(v=>v.classList.add("hidden"));$(`#${name}View`).classList.remove("hidden");$$("[data-view]").forEach(b=>b.classList.toggle("active",b.dataset.view===name));if(name==="dashboard")renderDashboard();scrollTo(0,0)}
...
function startExam(e){e?.preventDefault();let mode=$("#examMode").value,topic=$("#topicSelect").value,count=mode==="topic"?Number($("#topicQuestionCount").value):10,minutes=mode==="topic"?Number($("#topicTestMinutes").value):30;if(mode==="topic"&&(!validPositiveInteger(count)||!validPositiveInteger(minutes))){toast("Enter positive whole numbers for questions and minutes.");return}data.profile={name:$("#studentName").value.trim(),voice:$("#voiceMode").value,plan:$("#planLength").value,theme:$("#themeSelect").value,examMode:mode,topic,topicQuestionCount:String(count),topicTestMinutes:String(minutes)};exam={generatorVersion:GENERATOR_VERSION,mode,selectedTopic:mode==="topic"?topic:null,questionCount:count,timeLimitMinutes:minutes,questions:mode==="topic"?chooseTopicQuestions(topic,count):chooseQuestions(),answers:{},hints:{},assisted:{},chats:{},index:0,seconds:minutes*60,totalSeconds:minutes*60,startedAt:new Date().toISOString(),formulaOpens:0};data.draft=exam;save();applyProfile();showView("exam");$("#examStudent").textContent=data.profile.voice==="bestie"?`${data.profile.name}, let's cook.`:`${data.profile.name}'s exam`;renderQuestion();startTimer()}
function renderQuestion(){let q=exam.questions[exam.index],a=exam.answers[q.id];$("#questionTopic").textContent=q.topic;$("#questionDifficulty").textContent=q.difficulty;$("#questionType").textContent=q.type==="mc"?"MULTIPLE CHOICE":q.type==="tf"?"TRUE OR FALSE":"SHORT ANSWER";$("#questionPrompt").textContent=q.prompt;let area=$("#answerArea");if(q.type==="text")area.innerHTML=`<label><span class="kicker">YOUR ANSWER</span><input class="answer-input" id="textAnswer" value="${escapeHtml(a||"")}" placeholder="Type your answer here"></label>`;else area.innerHTML=`<div class="choice-list">${q.choices.map((c,i)=>`<label class="choice"><input type="radio" name="answer" value="${escapeHtml(c)}" ${a===c?"checked":""}><span><b>${String.fromCharCode(65+i)}.</b> ${c}</span></label>`).join("")}</div>`;$("#lesson").classList.add("hidden");$("#teachButton").textContent=data.profile.voice==="bestie"?"✦ Help bestie, teach me this":"✦ Teach me this topic";$("#progressText").textContent=`${exam.index+1} / ${exam.questions.length}`;$("#progressFill").style.width=`${(exam.index+1)/exam.questions.length*100}%`;$("#prevButton").disabled=exam.index===0;$("#nextButton").textContent=exam.index===exam.questions.length-1?"Submit exam →":"Next question →";$("#questionGrid").innerHTML=exam.questions.map((x,i)=>`<button class="${i===exam.index?"current":exam.answers[x.id]!=null?"answered":""}" data-index="${i}">${i+1}</button>`).join("");$("#questionGrid").querySelectorAll("button").forEach(b=>b.onclick=()=>{exam.index=+b.dataset.index;renderQuestion()});area.querySelectorAll("input").forEach(i=>i.oninput=()=>setAnswer(q.id,i.value))}
...
function next(){let q=exam.questions[exam.index];if(exam.answers[q.id]==null||exam.answers[q.id]===""){toast("Add an answer before moving on.");return}if(exam.index<exam.questions.length-1){exam.index++;renderQuestion()}else finishExam()}
...
function finishExam(timedOut=false){clearInterval(timerId);let correct=0,unassistedCorrect=0,unassistedTotal=0,topics={};exam.questions.forEach(q=>{let ok=isCorrect(q,exam.answers[q.id]);correct+=ok;topics[q.topic]??={correct:0,total:0};topics[q.topic].total++;topics[q.topic].correct+=ok;if(!exam.hints[q.id]){unassistedTotal++;unassistedCorrect+=ok}});Object.values(topics).forEach(t=>t.percent=Math.round(t.correct/t.total*100));let percent=Math.round(correct/exam.questions.length*100),result={id:Date.now(),date:new Date().toISOString(),mode:exam.mode,selectedTopic:exam.selectedTopic,questionCount:exam.questionCount,timeLimitMinutes:exam.timeLimitMinutes,score:correct,total:exam.questions.length,percent,grade:grade(percent),secondsUsed:exam.totalSeconds-exam.seconds,hints:Object.keys(exam.hints).length,assisted:Object.keys(exam.assisted||{}).length,formulaOpens:exam.formulaOpens,unassisted:unassistedTotal?Math.round(unassistedCorrect/unassistedTotal*100):0,topics,answers:exam.answers,hinted:exam.hints,questions:exam.questions.map(q=>q.id),timedOut};data.history.unshift(result);data.draft=null;save();renderResults(result);showView("results")}
function renderResults(r){let bestie=data.profile.voice==="bestie",weak=Object.entries(r.topics).sort((a,b)=>a[1].percent-b[1].percent)[0]?.[0];$("#resultTitle").textContent=bestie?(r.percent>=80?"Okayyy, you cooked.":"Practice data acquired ✨"):"Exam complete.";$("#resultMessage").textContent=bestie?`${data.profile.name}, you scored ${r.percent}%. ${weak} was the spicy bit today, so that’s our next glow-up target.`:`${data.profile.name}, you scored ${r.percent}%. Your next recommended focus is ${weak}.`;$("#resultPercent").textContent=`${r.percent}%`;$("#resultGrade").textContent=r.grade;$(".score-ring").style.setProperty("--score",`${r.percent*3.6}deg`);let mins=Math.floor(r.secondsUsed/60),secs=r.secondsUsed%60;$("#statCards").innerHTML=stats([["Points",`${r.score} / ${r.total}`],["Time used",`${mins}:${String(secs).padStart(2,"0")}`],["Hints / assisted",`${r.hints} / ${r.assisted||0}`],["Unassisted",`${r.unassisted}%`]]);$("#topicBreakdown").innerHTML=Object.entries(r.topics).map(([n,t])=>topicRow(n,t.percent)).join("");$("#nextSteps").innerHTML=Object.entries(r.topics).sort((a,b)=>a[1].percent-b[1].percent).slice(0,3).map(([n,t])=>`<div class="next-step"><strong>${n}</strong><span>${t.percent<60?"Relearn the core method, then try an easier example.":t.percent<80?"Do two focused practice questions.":"Keep it fresh with spaced review."}</span></div>`).join("");$("#reviewList").innerHTML=exam.questions.map((q,i)=>{let a=r.answers[q.id]||"No answer",ok=isCorrect(q,a);return `<div class="review-item"><p class="${ok?"correct-text":"wrong-text"}"><strong>${i+1}. ${ok?"Correct":"Review"} — ${q.topic}</strong></p><p>${q.prompt}</p><p>Your answer: ${escapeHtml(a)} · Correct answer: ${Array.isArray(q.answer)?q.answer[0]:q.answer}</p><p>${q.explanation}</p></div>`}).join("");$("#reviewList").classList.add("hidden");$("#toggleReview").textContent="Show review"}
...
function renderDashboard(){let h=data.history,name=data.profile.name||"Student";$("#dashboardTitle").textContent=h.length?`${name}, you're building momentum.`:"Your progress starts here.";let avg=h.length?Math.round(h.reduce((s,x)=>s+x.percent,0)/h.length):0,best=h.length?Math.max(...h.map(x=>x.percent)):0,streak=h.length;$("#dashboardStats").innerHTML=stats([["Exams completed",h.length],["Average score",`${avg}%`],["Personal best",`${best}%`],["Practice streak",`${streak} day${streak===1?"":"s"}`]]);let recent=[...h].slice(0,10).reverse();$("#scoreChart").innerHTML=recent.length?recent.map((r,i)=>`<div class="chart-col"><div class="chart-bar" style="height:${Math.max(r.percent*1.8,5)}px"><span>${r.percent}</span></div>D${h.length-recent.length+i+1}</div>`).join(""):`<p class="fine-print">Complete your first exam to see a trend.</p>`;let aggregate={};h.forEach(r=>Object.entries(r.topics).forEach(([n,t])=>{aggregate[n]??={c:0,n:0};aggregate[n].c+=t.correct;aggregate[n].n+=t.total}));$("#masteryList").innerHTML=Object.keys(aggregate).length?Object.entries(aggregate).sort((a,b)=>a[0].localeCompare(b[0])).map(([n,t])=>topicRow(n,Math.round(t.c/t.n*100))).join(""):`<p class="fine-print">Topic mastery will appear after your first exam.</p>`;$("#historyList").innerHTML=h.length?h.slice(0,8).map((r,i)=>`<div class="history-row"><strong>Exam ${h.length-i}</strong><span>${new Date(r.date).toLocaleDateString()}</span><span>${r.percent}% · ${r.grade}</span></div>`).join(""):`<p class="fine-print">No saved sessions yet. Your results stay on this device.</p>`}
